...
task("deploy-and-configure", "Deploy and configure all contracts")
  ...
  .setAction(async (args, { run }) => {
    // Deploy the Myths DAO contracts and return deployment information
    ...
    const executorAddress = contracts.MythsDAOExecutor.address;
    await contracts.MythsDescriptor.instance.transferOwnership(executorAddress);
    await contracts.MythsToken.instance.transferOwnership(executorAddress);
    await contracts.MythsAuctionHouseProxyAdmin.instance.transferOwnership(
      executorAddress
    );
    console.log(
    ...
    // to the Myths DAO executor.
    if (args.startAuction) {
      const auctionHouse = contracts.MythsAuctionHouse.instance.attach(
        contracts.MythsAuctionHouseProxy.address
      );
