...
const nameToFullyQualifiedName: Record<string, string> = {
  MythsAuctionHouseProxy: 'contracts/proxies/MythsAuctionHouseProxy.sol:MythsAuctionHouseProxy',
  MythsAuctionHouseProxyAdmin: 'contracts/proxies/MythsAuctionHouseProxyAdmin.sol:MythsAuctionHouseProxyAdmin',
};
...
task("verify-etherscan", "Verify the Solidity contracts on Etherscan")
  ...
  .setAction(
    async (
      { contracts }: { contracts: Record<ContractName, VerifyArgs> },
      hre
    ) => {
      for (const [name, contract] of Object.entries(contracts)) {
        ...
        try {
          ...
          if (code === "0x") {
            console.log(
              `${name} contract deployment has not completed. waiting to verify...`
            );
